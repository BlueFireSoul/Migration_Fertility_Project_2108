...
prog main
	...
	h2a_load_data
	h2b_estimate_migration_friction
	h2c_compile_puma_level_data
	h2d_obtain_met2013_inc
end

prog h2d_obtain_met2013_inc
	import delimited using "$puma1990_shapex_txt", clear
	keep statefip puma cbsafp shape_area
	merge n:1 statefip puma using $temp_dir/h2c_puma1990_inc.dta, nogen keep(match)
	...
	export delimited using $f2_output_dir/h2d_1990met2013_inc.csv, replace
end

prog h2c_compile_puma_level_data
	u $output_dir/h2a_census1990.dta, clear
	...
	ren popcount0 popL
	ren popcount1 popH
	merge 1:1 statefip puma using $temp_dir/h2a_puma_area.dta, nogen keep(match)
	save $temp_dir/h2c_puma1990_inc.dta, replace
end

prog h2b_estimate_migration_friction
	u $output_dir/h2a_census1990.dta, clear
	...
	quietly eststo: reg logpop v0 v1 v2 v3 v4 i.bpl i.metarea beta*, noconstant
	esttab, se keep(v*)
	esttab using $f2_output_dir/h2b_migration_cost_theta.csv, not noobs keep(v*) replace plain
end


prog h2a_load_data
	import delimited using "$puma1990_centroid_txt", clear
	ren latitude latitude_puma
	ren longitude longitude_puma
	keep puma statefip latitude_puma longitude_puma
	save $temp_dir/h2a_puma.dta, replace
	...
	merge n:1 bpl using $temp_dir/h2a_state.dta, nogen keep(match)
	merge n:1 statefip puma using $temp_dir/h2a_puma.dta, nogen keep(match)
	geodist latitude_bpl longitude_bpl latitude_puma longitude_puma, generate(geodistance_puma)
	save $output_dir/h2a_census1990.dta, replace
end
